...
\begin{enumerate}
    ...
    \item For each $u \in [0,w)$, generate a modified Chaum-Pedersen proof, where we additionally bind $\mu$ to the initial transcript: $$(\Pi_{\text{chaum}})_u = \func{ChaumProve}(pp_{\text{chaum}}, \mu, S_u', T_u; (s_u, r, -\hash_{\text{ser}'}(s_u, D)))$$
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Compute the binding hash $\mu$ as before, check that $$\func{ChaumVerify}(pp_{\text{chaum}},\mu,(\Pi_{\text{chaum}})_u,S_u',T_u),$$ and output 0 if this fails.
    \end{enumerate}
