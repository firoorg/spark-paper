...
\item Query $\func{AddrTable}$ on the value $S - \dcom(\hash_{\text{ser}}(K_{\text{der}}),0,0)$ to obtain the diversifier index $i$; if this query fails, return failure.
...
\begin{enumerate}
    ...
    \item For each $u \in [0,w)$, generate a modified Chaum-Pedersen proof, where we additionally bind $\mu$ to the initial transcript: $$(\Pi_{\text{chaum}})_u = \func{ChaumProve}(pp_{\text{chaum}},S_u', T_u; (s_u, r, -\hash_{\text{ser}'}(s_u, D)))$$
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Check that $\func{ChaumVerify}(pp_{\text{chaum}},(\Pi_{\text{chaum}})_u,S_u',T_u)$, and output 0 if this fails.
    \end{enumerate}
...
\begin{enumerate}
    ...
    \end{align*}
\end{enumerate}
Using the tuple $(s_1,s_2,D)$, any player can additionally compute the base key components $P_1 = \dcom(s_1,0,0)$ and $P_2 = \dcom(s_2,0,0) + D$.
...
Further, assume that each nonce list $L_\alpha$ contains $k + 1$ unused nonces.
...
\begin{enumerate}
    \item Parses the next available set of nonces $(d_{\beta,k}G,e_{\beta,k}G)$ in each $L_\beta$ for $1 \leq \beta \leq t$ (and removes them from each list after use) to compute the following value:
    $$\rho = \hash_{\rho}(\{\beta,d_{\beta,k}G,e_{\beta,k}G\}_{\beta=1}^{t},\mu,S_u',T_u)$$
    \item Sets the initial proof commitments:
    \begin{align*}
        A_1 &= \hash_F(\rho)F + \hash_H(\rho)H + \sum_{\beta=1}^t \left( d_{\beta,k}G + \rho e_{\beta,k}G \right) \\
        A_2 &= \hash_F(\rho)T_u + \sum_{\beta=1}^t \left( d_{\beta,k}G + \rho e_{\beta,k}G \right)
    \end{align*}
    \item Computes the challenge $c$ using the proof transcript as in the original $\text{Spend}$ description.
    \item Computes its Lagrange coefficient $$\lambda_\alpha = \prod_{\beta=1,\beta \neq \alpha}^t \left( \frac{\beta}{\beta-\alpha} \right)$$ and the response share $t_{2,\alpha} = d_{\alpha,k} + \rho e_{\alpha,k} + \lambda_\alpha r_\alpha c$, and sends $t_{2,\alpha}$ to the other signing players.
    \item On receipt of $t_{2,\beta}$ from another player, checks that $$t_{2,\beta}G = d_{\beta,k}G + \rho e_{\beta,k}G + c\lambda_{\beta} \sum_{\gamma=1}^{\nu}\sum_{j=0}^{t-1} \beta^j C_{\gamma,j}$$ and aborts otherwise.
    \item On receipt of all $t_{2,\beta}$ values, computes the proof responses:
    \begin{align*}
        t_1 &= \hash_F(\rho) + cs_u \\
        t_2 &= \sum_{\beta=1}^t t_{2,\beta} \\
        t_3 &= \hash_H(\rho) - c\hash_{\text{ser}'}(s_u,D)
    \end{align*}
\end{enumerate}
...
Observe that a valid spend binds all transaction elements except for modified Chaum-Pedersen proofs into each such proof via $\hash_{\text{bind}}$ and the proof transcripts.
...
\begin{enumerate}
    ...
    $$\Pi_{\text{dl}} = \func{ParallelRepProve}(pp_{\text{dcom}},K,kQ_0,kQ_1,kG,Q_0,Q_1 ; k)$$
...
\begin{enumerate}
    ...
    $$\func{ParallelRepVerify}(pp_{\text{dcom}},\Pi_{\text{dl}},K,K_{\text{der}},K_{\text{div}},Q_0,Q_1)$$
    ...
    \item Checks that $S = \dcom(\hash_{\text{ser}}(K_{\text{der}}),0,0) + Q_2$, and aborts otherwise.
