...
The construction itself is a tuple of algorithms $(\text{SymKeyGen},\text{SymEncrypt},\text{SymDecrypt})$.
Here $\text{SymKeyGen}$ is a key derivation function that accepts as input an arbitrary string, and produces a key in the appropriate key space.
The algorithm $\text{SymEncrypt}$ accepts as input a key and arbitrary message string, and produces ciphertext in the appropriate space.
The algorithm $\text{SymDecrypt}$ accepts as input a key and ciphertext string, and produces a message in the appropriate space if authentication succeeds.
...
\item Generate a symmetric encryption key $k_{\text{sym}} = \text{SymKeyGen}(kQ_1)$; encrypt the value $\overline{v} = \text{SymEnc}(k_{\text{sym}},v)$ (if $b=0$) and memo $\overline{m} = \text{SymEnc}(k_{\text{sym}},m)$.
...
\item Generate a symmetric encryption key $k_{\text{sym}} = \text{SymKeyGen}(s_1 K)$; decrypt the value $v = \text{SymDec}(k_{\text{sym}},\overline{v})$ (if $b=0$) and memo $m = \text{SymDec}(k_{\text{sym}},\overline{m})$.
...
\item Parse the serial number commitment $S$, value commitment $C$, recovery key $K$, encrypted value $\overline{v}$ (if the coin is of type $b=0$) or value $v$ (if the coin is of type $b=1$), and encrypted memo $\overline{m}$ from $\text{Coin}$.
\item If $\com(\hash_{\text{ser}}(s_1 K,Q_1,Q_2), 0) + Q_2 \neq S$, return failure.
\item Generate a symmetric encryption key $k_{\text{sym}} = \text{SymKeyGen}(s_1 K)$; decrypt the value $v = \text{SymDec}(k_{\text{sym}},\overline{v})$ (if $b=1$) and memo $m = \text{SymDec}(k_{\text{sym}},\overline{m})$.
\item If $\com(v,\hash_{\text{val}}(s_1 K)) \neq C$, return failure.
...
Since the generating transaction is valid, we have an extracted opening $C = vG + aF$ from either the balance proof (in a \text{Mint} transasction) or the range proof (in a \text{Spend} transaction).
...
That is, $\mathcal{A}$ produces a \text{Spend} transaction $\text{tx}_{\text{spend}}$ by an \text{Insert} question that is valid on the augmented ledger $(L, \vec{a})$ and consumes a coin can be recovered by a public address $(Q_1, Q_2) \in \text{ADDR}$.
...
However, $S_l$ was generated in a transaction in $L'$ preceding $\text{tx}$, and contains either a proof of representation (if generated in a \text{Mint} transaction) or range proof (if generated in a \text{Spend} transaction) that asserts its representation with respect to the generators $G,F$ is unique up to the binding property of the commitment scheme.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item Samples a random input used to produce a symmetric encryption key $\text{SymKeyGen} \to k_{\text{enc}}$.
            \item Simulates the memo encryption by selecting random $\widetilde{m}$ of the proper length, and encrypting it to produce $\text{SymEnc}(k_{\text{enc}}, \widetilde{m}) \to \overline{m}$.
        \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \begin{enumerate}
                ...
                \item Samples a random input used to produce a symmetric encryption key $\text{SymKeyGen} \to k_{\text{enc}}$.
                \item Simulates the value encryption by selecting random $\widetilde{v}$ of the proper length, and encrypting it to produce $\text{SymEnc}(k_{\text{enc}}, \widetilde{v}) \to \overline{v}_j$.
                \item Simulates the memo encryption by selecting random $\widetilde{m}$ of the proper length, and encrypting it to produce $\text{SymEnc}(k_{\text{enc}}, \widetilde{m}) \to \overline{m}_j$.
