...
\item Query $\text{AddrTable}$ on the value $S - \dcom(\hash_{\text{ser}}(K_{\text{der}}),0,0)$ to obtain the diversifier index $i$; if this query fails, return failure.
...
Sample cryptographic hash functions $$\hash_{\text{pok}},\hash_{s_1},\hash_{s_2},\hash_{\rho},\hash_{\rho_F},\hash_{\rho_H}: \{0,1\}^* \to \F$$ uniformly at random.
...
\begin{enumerate}
    \item Selects a set of coefficients $\{a_{\alpha,j}\}_{j=0}^{t-1} \subset \F$ uniformly at random.
    ...
    \end{enumerate}
    \item For each $1 \leq \beta \leq \nu$, computes a player share $$\widehat{r}_{\alpha,\beta} = \sum_{j=0}^{t-1} a_{\alpha,j}\beta^j$$ and sends it to player $\beta$.
...
\begin{enumerate}
    \item Parses the next available set of nonces $(d_{\beta,k}G,e_{\beta,k}G)$ in each $L_\beta$ for $1 \leq \beta \leq t$ (and removes them from each list after use) to compute, for all $1 \leq \beta \leq t$, the following value:
    $$\rho_\beta = \hash_{\rho}(\{d_{\gamma,k}G,e_{\gamma,k}G\}_{\gamma=1}^{t},\beta,S_u',T_u)$$
    \item Sets the initial proof commitments:
    \begin{align*}
        A_1 &= \sum_{\beta=1}^t \left( \hash_{\rho_F}(\rho_\beta)F + d_{\beta,k}G + \rho_\beta e_{\beta,k}G + \hash_{\rho_H}(\rho_\beta)H \right) \\
        A_2 &= \sum_{\beta=1}^t \left( \hash_{\rho_F}(\rho_\beta)T + d_{\beta,k}G + \rho_\beta e_{\beta,k}G \right)
    \end{align*}
    \item Computes the challenge $c$ using the proof transcript as in the original $\text{Spend}$ description.
    \item Computes its Lagrange coefficient $$\lambda_\alpha = \prod_{\beta=1,\beta \neq \alpha}^t \left( \frac{\beta}{\beta-\alpha} \right)$$ and the response share $t_{2,i} = d_{\alpha,k} + \rho_\alpha e_{\alpha,k} + \lambda_\alpha r_\alpha c$, and sends $t_{2,\alpha}$ to the other signing players.
    \item On receipt of $t_{2,\beta}$ from another player, checks that $$t_{2,\beta}G = d_{\beta,k}G + \rho_B e_{\beta,k}G + c\lambda_{\beta} \sum_{\gamma=1}^{\nu}\sum_{j=0}^{t-1} \beta^j C_{\gamma,j}$$ and aborts otherwise.
    \item On receipt of all $t_{2,\beta}$ values, computes the proof responses:
    \begin{align*}
        t_1 &= cs_u + \sum_{\beta=1}^t \hash_{\rho_F}(\rho_\beta) \\
        t_2 &= \sum_{\beta=1}^t t_{2,\beta} \\
        t_3 &= \sum_{\beta=1}^t \hash_{\rho_H}(\rho_\beta) - c\hash_{\text{ser}'}(s_u,D)
    \end{align*}
\end{enumerate}
