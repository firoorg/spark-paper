...
\item $\func{Mint}$: This algorithm produces a transaction transferring public value to a recipient public address.
...
\begin{enumerate}
\item Run $\func{CreateCoin}(\addr_{\text{pk}}, v, m, 1)$ to obtain coin $\func{Coin} = (S, K, C, v, \overline{r})$ and nonce $k$.
\item Generate a value representation proof on the value commitment: $$\Pi_{\text{bal}} = \func{RepProve}(pp_{\text{rep}},H,C - \com(v,0); \hash_{\text{val}}(k))$$
\item Output the mint transaction $\func{tx}_{\text{mint}} = (\func{Coin}, \Pi_{\text{bal}})$.
\end{enumerate}
...
Let $\func{MatrixCom}: \F^{mn} \times \F \to \G$ be an additively-homomorphic matrix commitment construction that is perfectly hiding and computationally binding.
...
\begin{enumerate}
    ...
    \begin{align*}
        A &\equiv \func{MatrixCom}(\{a_{j,i}\}_{j,i=0}^{m-1,n-1}, r_A) \\
        B &\equiv \func{MatrixCom}(\{\delta(l_{j},i)\}_{j,i=0}^{m-1,n-1}, r_B) \\
        C &\equiv \func{MatrixCom}(\lbrace a_{j,i}(1-2\delta(l_j,i))\rbrace_{j,i=0}^{m-1,n-1}, r_C) \\
        D &\equiv \func{MatrixCom}(\{-a_{j,i}^2\}_{j,i=0}^{m-1,n-1}, r_D)
    \end{align*}
    ...
    \begin{align*}
        A + xB &= \func{MatrixCom}(\lbrace f_{j,i} \rbrace_{j,i=0}^{m-1,n-1}, z_A) \\
        xC + D &= \func{MatrixCom}(\lbrace f_{j,i}(x - f_{j,i})\rbrace_{j,i=0}^{m-1,n-1}, z_C) \\
    \end{align*}
    and
    \begin{align*}
